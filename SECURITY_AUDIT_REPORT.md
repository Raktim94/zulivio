...
- **Status:** Open / accepted risk — not addressed this session. Recommendation: application-level envelope encryption (encrypt at rest with a key from an env var, decrypt only when the backup job runs) before Phase 2. Flagged here explicitly rather than silently left as-is.
