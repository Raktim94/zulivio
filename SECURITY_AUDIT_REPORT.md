...
- **Not covered by this fix:** `GoogleSheetsConfig.privateKey` has the identical plaintext-at-rest pattern (added in an unrelated, still-uncommitted in-progress change to this repo) — out of scope for this pass since it wasn't part of the original finding, but the same `FieldEncryptionService` should be wired into it too when that feature is finished.
- **Status:** Fixed.
