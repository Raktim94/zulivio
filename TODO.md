...
- [x] Playwright pass across employee + Sales Head flows at 375/768/1440px, accessibility check — run 2026-08-18 via a standalone script (`playwright` npm package + cached Chromium binary, since the Playwright MCP server itself still wasn't reachable in-session) driving a throwaway Postgres + local dev servers. Bootstrapped an org, logged in as Master Owner, screenshotted 10 routes at 375/768/1440px + mobile nav drawer. Found 4 categories of real issues (below), fixed all of them, then re-ran: **0 overflow failures, 0 axe violations (wcag2a/2aa/21a/21aa) across all 10 routes, 0 console errors, all HTTP 200.**
  - **Fixed — horizontal page overflow** on `/employees` + `/settings` (375px) and `/sales-head/employees` (375px + 768px): the flex-1 content column in `app/(app)/layout.tsx` had no `min-w-0`, so it wouldn't shrink below its content's natural width and the whole page scrolled sideways instead of the `Table`/`Tabs` components' own contained scroll. Added `min-w-0` to that column and `<main>`, plus `overflow-x-auto` + `shrink-0` to the `Tabs` component itself (same root-cause class of bug, would've hit any page with 5+ tabs at narrow widths).
  - **Fixed — accessibility, color-contrast**: sidebar nav-group labels (`text-white/40` → `/60`) and the `MadeBy` footer's dark variant (same fix) — both were 3.67:1, under the 4.5:1 AA minimum. `--color-muted` design token (`#64748b`, used for the top-bar subtitle and every unselected tab label) measured 4.42:1 against `--color-canvas`, also under threshold — darkened to `#475569` (Tailwind slate-600, ~7:1) at the token level so every usage is fixed at once.
  - **Fixed — accessibility, missing accessible names**: `/employees`'s "Add employee" form `<select>` (critical, `select-name`) plus the 3 sibling `<Input>`s (same gap, placeholder-only, not individually flagged by axe but the same real issue) — added `aria-label` to all four fields.
  - **Fixed — accessibility, systemic brand-color contrast** (found during the fix-verification re-scan, not in the original report): the primary brand green `--color-emerald` (`#168b65`) measured 4.27:1 both as white-on-emerald (every primary `Button`, sitewide) and as emerald-text-on-white (8+ links app-wide) — under 4.5:1. This is a real brand-color decision, not a scoped bug, so it was surfaced to the user before touching it; approved fix applied: promoted the existing `--color-emerald-dark` shade (`#106e51`, ~6.2:1) to be the default `--color-emerald`, and shifted `--color-emerald-dark` one step further down (`#0d5841`) so the hover-darken interaction still reads as a real state change. Only the light-theme token block was touched — dark-theme's separate emerald values were untested (axe ran under the default/light render) and are out of scope here.
